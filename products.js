...
async function loadScopeList() {
  try {
    const json = await apiGet('/scopes');
    if (json.code === 0 && json.data) {
      scopeList = json.data;
      scopeLoaded = true;
      renderPScopeTable();
    ...
    console.error('适用部位加载失败:', e);
    showToast('适用部位加载失败，请确认后端服务已启动', 'error');
  }
}

function renderScopeTable() {
  if (!scopeLoaded) return;
  const searchText = (document.getElementById('scopeSearchInput')?.value || '').trim().toLowerCase();
  let filtered = scopeList;
  if (searchText) filtered = filtered.filter(s => s.name.toLowerCase().includes(searchText));
  document.getElementById('scopeTableBody').innerHTML = filtered.map(s =>
    `<tr><td>${s.name}</td><td><span class="p-tag ${s.status === '启用' ? 'enabled' : 'disabled'}">${s.status}</span></td><td><span class="p-action-link" onclick="openScopeModal(${s.id})">修改</span><span class="p-action-link danger" onclick="deleteScope(${s.id})">删除</span></td></tr>`
  ).join('');
  document.getElementById('scopeTotalInfo').textContent = `共 ${filtered.length} 条数据`;
}

function renderPScopeTable() {
  if (!scopeLoaded) return;
  const searchText = (document.getElementById('pScopeSearchInput')?.value || '').trim().toLowerCase();
  let filtered = scopeList;
  if (searchText) filtered = filtered.filter(s => s.name.toLowerCase().includes(searchText));
  const tbody = document.getElementById('pScopeTableBody');
  if (!tbody) return;
  tbody.innerHTML = filtered.map(s =>
    `<tr><td>${s.name}</td><td><span class="p-tag ${s.status === '启用' ? 'enabled' : 'disabled'}">${s.status}</span></td><td><span class="p-action-link" onclick="openScopeModal(${s.id})">修改</span><span class="p-action-link danger" onclick="deleteScope(${s.id})">删除</span></td></tr>`
  ).join('');
  const info = document.getElementById('pScopeTotalInfo');
  if (info) info.textContent = `共 ${filtered.length} 条数据`;
}

function filterScopeTable() { renderScopeTable(); }
function filterPScopeTable() { renderPScopeTable(); }
function resetScopeFilter() {
  document.getElementById('scopeSearchInput').value = '';
  renderScopeTable();
}
function resetPScopeFilter() {
  document.getElementById('pScopeSearchInput').value = '';
  renderPScopeTable();
}

let scopeEditIndex = -1;
function openScopeModal(id) {
  scopeEditIndex = typeof id === 'number' ? id : -1;
  const overlay = document.getElementById('scopeModalOverlay');
  document.getElementById('scopeModalTitle').textContent = scopeEditIndex > 0 ? '修改部位' : '新增部位';
  if (scopeEditIndex > 0) {
    const item = scopeList.find(s => s.id === scopeEditIndex);
    if (item) {
      document.getElementById('scopeInputName').value = item.name;
      document.getElementById('scopeInputStatus').value = item.status;
    }
  } else {
    document.getElementById('scopeInputName').value = '';
    document.getElementById('scopeInputStatus').value = '启用';
  }
  overlay.classList.add('open');
  document.getElementById('scopeInputName').focus();
}

function closeScopeModal() {
  document.getElementById('scopeModalOverlay').classList.remove('open');
}

async function saveScope() {
  const name = document.getElementById('scopeInputName').value.trim();
  const status = document.getElementById('scopeInputStatus').value;
  if (!name) {
    showToast('请输入部位名称', 'error');
    document.getElementById('scopeInputName').focus();
    return;
  }
  try {
    if (scopeEditIndex > 0) {
      const json = await apiPut(`/scopes/${scopeEditIndex}`, { name, status });
      if (json.code === 0) {
        showToast('修改成功');
      } else {
        showToast(json.msg || '修改失败', 'error');
      ...
      const json = await apiPost('/scopes', { name });
      if (json.code === 0) {
        showToast('新增成功');
      } else {
        showToast(json.msg || '新增失败', 'error');
    ...
    console.error('部位保存失败:', e);
    showToast('保存失败，请确认后端服务已启动', 'error');
  }
}

async function deleteScope(id) {
  const item = scopeList.find(s => s.id === id);
  if (!item) return;
  if (!confirm(`确定要删除部位「${item.name}」吗？`)) return;
  try {
    const json = await apiDelete(`/scopes/${id}`);
    if (json.code === 0) {
      showToast('删除成功');
      await loadScopeList();
    } else {
...
function switchProductTab(tabName) {
  ...
  switch(tabName) {
    ...
    case 'params':
      if (typeList.length > 0) renderTypeTable();
      if (brandLoaded) renderBrandTable();
      if (specUnits.length > 0) renderSpecUnits();
      if (scopeLoaded) renderPScopeTable();
      break;
...
/* ========== 产品参数子标签切换 ========== */
function switchParamsSubTab(tabName) {
  document.querySelectorAll('.p-subtab-btn').forEach(el => {
    el.classList.toggle('active', el.dataset.paramsTab === tabName);
  });
  document.querySelectorAll('.p-params-subpage').forEach(el => {
    el.classList.toggle('active', el.dataset.paramsTab === tabName);
  });
  // 确保数据已加载
  switch(tabName) {
    ...
    case 'scope': if (scopeLoaded) renderPScopeTable(); break;
  }
}
