...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_api_get(self, path, params):
        ...
        try:
            if path == '/api/brands':
                ...
                c.execute("SELECT id, name, status FROM scopes ORDER BY id")
                rows = [dict(r) for r in c.fetchall()]
                self._send({'code': 0, 'data': rows})
    ...
    def do_POST(self):
        ...
        try:
            if path == '/api/brands':
                ...
                name = body.get('name', '').strip()
                if not name:
                    self._send({'code': 1, 'msg': '部位名称不能为空'})
                    return
                try:
                    c.execute("INSERT INTO scopes (name, status) VALUES (?, '启用')", (name,))
                    conn.commit()
                    self._send({'code': 0, 'msg': '新增成功', 'data': {'id': c.lastrowid}})
                except sqlite3.IntegrityError:
                    self._send({'code': 2, 'msg': '部位已存在'})
    ...
    def do_PUT(self):
        ...
        try:
            ...
            elif path.startswith('/api/scopes/'):
                scope_id = path.split('/')[-1]
                name = body.get('name', '').strip()
                status = body.get('status')
                if name:
                    try:
                        c.execute("UPDATE scopes SET name=? WHERE id=?", (name, scope_id))
                    except sqlite3.IntegrityError:
                        self._send({'code': 2, 'msg': '部位已存在'})
                        return
                if status:
                    c.execute("UPDATE scopes SET status=? WHERE id=?", (status, scope_id))
                conn.commit()
                self._send({'code': 0, 'msg': '更新成功'})
    ...
    def do_DELETE(self):
        ...
        try:
            if path.startswith('/api/brands/'):
                ...
                scope_id = path.split('/')[-1]
                c.execute("DELETE FROM scopes WHERE id=?", (scope_id,))
                conn.commit()
                self._send({'code': 0, 'msg': '删除成功'})
